import {
  ...
  export class JobsController {
    ...
    // ParseIntPipe -- using this the paramter will get converted from string to the number
    @Get(":id")
    // using this mehtod we can customize according to our need 
    // findJobByid(@Param('id', new ParseIntPipe({errorHttpStatusCode: HttpStatus.NOT_ACCEPTABLE})) id: number){
      // so yha hme khud se initiate krna pda hme parse int pipe ko 
      // return this.jobsService.findById(id);
    // }

    // In this method it is the responsibiltity of nest js to make instance of ParseIntPipe and then inject that pipe.
